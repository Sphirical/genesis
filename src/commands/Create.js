...
class Create extends Command {
  ...
  constructor(bot) {
    ...
    this.regex = new RegExp(`^${this.call}\\s?(room|raid|team)?`, 'i');
  ...
  run(message) {
    ...
    if (type) {
      ...
      if (roomType === 'room' || roomType === 'raid' || roomType === 'team') {
        ...
        const name = `${type}-${message.member.displayName}`.toLowerCase();
          ...
          message.guild.createChannel(name, 'text')
            ...
            .then((params) => {
              ...
              return params[0].then((voiceChannel) => {
                ...
                this.addToListenedChannels(voiceChannel, textChannel);
  ...
  /**
   * Add text and voice channels to listened channels
   * @param {TextChannel} textChannel text channel to add
   * @param {VoiceChannel} voiceChannel voice channel to add
   */
  addToListenedChannels(textChannel, voiceChannel) {
    this.bot.listenedChannels.push({
      voiceChannel,
      textChannel,
      timeout: setTimeout((createdChannel, user) => {
        if (createdChannel.permissionsFor(user).hasPermission('MANAGE_CHANNELS')) {
          createdChannel.delete();
        }
      }, 300000, voiceChannel, this.bot.client.user),
    });
  }
  ...
  sendInvites(voiceChannel, users, author) {
    if (voiceChannel.permissionsFor(this.bot.client.user).hasPermission('CREATE_INSTANT_INVITE')) {
      users.forEach((user) => {
        voiceChannel.createInvite({ maxUses: 1 }).then((invite) => {
          user.sendMessage(`Invite for ${voiceChannel.name} from ${author}: ${invite}`);
        });
      });
    }
  }
