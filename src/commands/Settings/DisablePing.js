...
class EnablePingEvent extends Command {
  constructor(bot) {
    super(bot, 'settings.ping.disable', 'ping off');
    this.usages = [
      { description: 'Show command for pinging for items', parameters: [] },
      { description: 'Disable pinging for an event(s) or item(s)', parameters: ['event(s) or item(s) to disable ping for'] },
    ];
  ...
  run(message) {
    ...
    if (savePing) {
      eventsToPing.forEach(event => promises.push(this.bot.settings
        .setEventTypePing(message.channel, event, false)));
